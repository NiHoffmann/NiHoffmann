...
- 🛠️ Currently at **Fraunhofer FKIE** - **Sole owner** of a **mobile sensor platform**, redesigned from the ground up (replacing a prior Raspberry Pi-based system) for **5-6x better energy efficiency**: hardware, firmware, case design, GUI tooling, and testing & assembly. ~120µA deep sleep (RTC-gated) / ~400µA light sleep (accelerometer-gated) via tickless FreeRTOS and split power domains - limited by the LTE modem's and GPS's base current draw. (Not open source, unfortunately, so that's all I can share here 😅)
